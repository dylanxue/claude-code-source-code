...
- 第一阶段先支持“用户附图给模型看”
- 第二阶段再支持“模型调用工具读取图片，并把图片作为结构化结果回给模型”
...
- 模型调用工具读取图片并以 `image block` 回传给 LLM
...
### 4.2 P0：QueryEngine 输入面支持结构化用户消息
...
### 4.3 P0：Anthropic provider 打通图片透传
...
### 4.4 P0：OpenAI Responses 主路径支持图片输入
...
### 4.5 P0：显式收紧 OpenAI chat-completions 边界
...
### 4.7 P1：会话持久化与 transcript 观察面补齐
...
- `src/tools/WebFetch*` 或新增远程图片读取工具
- `src/core/queryLoop.ts`
- 视需要新增轻量图片下载/识别模块
...
1. `P0` 消息模型
2. `P0` QueryEngine 输入扩展
3. `P0` Anthropic provider
4. `P0` OpenAI Responses
5. `P0` OpenAI chat-completions 边界收紧
6. `P1` headless `--image`
7. `P1` session / transcript / verbose 收口
8. `P2` interactive 输入层重构准备
9. `P2` tool result 结构化多模态支持
10. `P2` 本地图片读取工具链路
11. `P2` 远程图片读取工具链路
