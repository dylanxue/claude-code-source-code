...
async function startLocalWebFixtureServer() {
  const server = createServer((request, response) => {
    ...
    if (url.pathname === "/page") {
      ...
      response.end(
        [
          "<html><head><title>Lesson 13 Fixture</title></head><body>",
          "<h1>Lesson 13 Fixture</h1>",
    ...
    if (url.pathname === "/search") {
      ...
      response.end(
        [
          "<html><body>",
          `<a class="result__a" href="/l/?uddg=${encodeURIComponent(`http://127.0.0.1:${port}/page`)}">Lesson 13 Fixture</a>`,
...
async function run() {
  ...
  assert(/Lesson 13 Fixture/.test(webFetch.result), "WebFetch should summarize the fetched page", webFetch);
  ...
  const webSearch = await webSearchTool.execute(
    {
      query: "lesson13 fixture",
      blocked_domains: ["blocked.example.com"],
    },
    { workspaceRoot },
  );
  const webSearchHits = webSearch.results?.[1]?.content ?? [];
  assert(webSearch.query === "lesson13 fixture", "WebSearch should echo the query", webSearch);
