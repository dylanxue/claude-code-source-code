...
function defaultSandboxConfig() {
  return {
    ...
    redirect_home_tmpdir: parseBooleanEnv(
      process.env.BASH_SANDBOX_REDIRECT_HOME_TMPDIR,
      false,
    ),
  };
}
...
function sandboxHomePath(workspaceRoot) {
  return path.join(workspaceRoot, ".sandbox-home");
}

function sandboxTmpPath(workspaceRoot) {
  return path.join(workspaceRoot, ".sandbox-tmp");
}
...
async function buildSandboxStatus(input, workspaceRoot) {
  ...
  const requested = {
    ...
    redirect_home_tmpdir: config.redirect_home_tmpdir,
  };
  ...
  return {
    ...
    home_tmpdir_redirect_active:
      requested.enabled && filesystemActive && requested.redirect_home_tmpdir,
...
async function prepareSandboxDirs(workspaceRoot) {
  await mkdir(sandboxHomePath(workspaceRoot), { recursive: true });
  await mkdir(sandboxTmpPath(workspaceRoot), { recursive: true });
}
...
function buildLinuxSandboxLauncher(command, workspaceRoot, sandboxStatus) {
  ...
  if (sandboxStatus.home_tmpdir_redirect_active) {
    envOverrides.HOME = sandboxHomePath(workspaceRoot);
    envOverrides.TMPDIR = sandboxTmpPath(workspaceRoot);
  }
...
async function prepareCommandExecution(command, workspaceRoot, sandboxStatus, { createDirs }) {
  if (createDirs && sandboxStatus.home_tmpdir_redirect_active) {
    await prepareSandboxDirs(workspaceRoot);
  }
  ...
  if (sandboxStatus.home_tmpdir_redirect_active) {
    env.HOME = sandboxHomePath(workspaceRoot);
    env.TMPDIR = sandboxTmpPath(workspaceRoot);
  }
