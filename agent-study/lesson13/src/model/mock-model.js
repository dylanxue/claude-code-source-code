...
function extractSkillName(text) {
  ...
  const explicit = text.match(/(?:skill|技能)\s+([A-Za-z0-9_./$-]+)/i)?.[1];
  if (explicit) {
...
function summarizeToolResult(lastToolResult) {
  ...
  if (lastToolResult.content.toolName === "Skill") {
    return [
      `我已经加载了 Skill \`${payload.skill}\`。`,
      "",
      `路径：${payload.path}`,
      payload.description ? `描述：${payload.description}` : "描述：无",
      payload.args ? `参数：${payload.args}` : "参数：无",
      "",
      "这一步对应真实 coding agent 里的可复用工作流层：当某类任务已经有稳定套路时，不必每次临时现编，而是可以先加载对应 Skill。",
    ].join("\n");
  }
  ...
  async decide({ messages, iteration }) {
    ...
    if (!lastToolResult) {
      if (
        ...
        userText.includes("技能") ||
        userText.includes("工作流模板")
      ) {
        return {
          type: "tool",
          toolName: "Skill",
          input: {
            skill: extractSkillName(userText),
          },
        };
      }
