...
const originalEnv = {
  ...
  BASH_SANDBOX_REDIRECT_HOME_TMPDIR: process.env.BASH_SANDBOX_REDIRECT_HOME_TMPDIR,
...
test("bash preserves HOME and TMPDIR by default to stay closer to Claude Code", async () => {
  const result = await bashTool.execute(
    {
  ...
  assert.equal(parsed.home, process.env.HOME ?? null);
  assert.equal(parsed.tmpdir, process.env.TMPDIR ?? null);
});

test("bash can still redirect HOME and TMPDIR when explicitly configured", async () => {
  setEnv({
    BASH_SANDBOX_REDIRECT_HOME_TMPDIR: "true",
  });
  ...
  assert.equal(result.sandboxStatus.home_tmpdir_redirect_active, true);
});
...
test("bash reports claw-code style sandboxStatus defaults", async () => {
  ...
  assert.equal(result.sandboxStatus.requested.redirect_home_tmpdir, false);
  assert.deepEqual(result.sandboxStatus.requested.allowed_mounts, []);
  assert.deepEqual(result.sandboxStatus.allowed_mounts, []);
  assert.equal(result.sandboxStatus.home_tmpdir_redirect_active, false);
});
...
test("bash does not override HOME and TMPDIR when sandbox is disabled", async () => {
  const result = await bashTool.execute(
    {
      command: 'node -p "JSON.stringify({home: process.env.HOME, tmpdir: process.env.TMPDIR})"',
      dangerouslyDisableSandbox: true,
...
test("bash keeps the direct file dump guardrail in lesson13", async () => {
  setEnv({
...
test("runtime can ignore bash workspace boundary blocks via config", () => {
  setEnv({
