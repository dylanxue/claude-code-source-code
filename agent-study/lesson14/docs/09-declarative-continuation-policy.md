...
- 如果 lesson13 显式设置了 `TOOLS_IGNORE_WORKSPACE_BOUNDARY=true`，这层文件工具 boundary 会整体关闭
...
另外 lesson13 现在也不只是“回传 sandboxStatus”：

- 默认会保留真实的 `HOME / TMPDIR`，这点更接近 Claude Code 公共文档里的 OS-level sandbox 口径
- 只有显式设置 `BASH_SANDBOX_REDIRECT_HOME_TMPDIR=true` 时，才会把 `HOME / TMPDIR` 切到 `.sandbox-home / .sandbox-tmp`
...
同样地，如果 lesson13 显式设置了 `TOOLS_IGNORE_WORKSPACE_BOUNDARY=true`：

- 文件工具的 workspace boundary 检查会整体跳过
- `bash` 的这条 workspace escape pre-tool guard 也会一起关闭
