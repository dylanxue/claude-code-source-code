...
async function main() {
  ...
  const runtimeDecisionDebug = process.env.RUNTIME_DECISION_DEBUG === "true";
  ...
  if (!prompt) {
    ...
    console.log('  npm start -- --resume .dclaw/sessions/<workspace-hash>/session-xxx.json "继续刚才的任务"');
    console.log('  npm start -- "查找 src 里和 tool 相关的内容，并总结一下"');
    console.log('  npm start -- "运行 `pwd`"');
    console.log('  npm start -- "我想找一个适合搜索工具能力的 tool，用什么工具？"');
    console.log('  npm start -- "请加载 review skill"');
    console.log('  npm start -- "请委托一个 Explore subagent 去阅读 README 并总结项目目标"');
    console.log('  npm start -- "禁止直接使用 Agent。请先 WorkerCreate 一个 Explore worker，等 ready_for_prompt 后再 WorkerSendPrompt，让它读取 README.md 第一行并总结"');
    console.log('  npm start -- "搜索网络上关于 Model Context Protocol 的最新资料"');
    console.log('  npm start -- "抓取网页 https://example.com 并总结内容"');
  ...
  const app = createAgentApp({
    ...
    runtimeCallbacks: {
      onRuntimeEvent(event) {
        ...
        if (event.channel === "assistant_stream" && event.type === "model_retry") {
          writeStreamingMeta(
            `[model_retry] provider=${event.provider ?? "unknown"} attempt=${event.attempt} next=${event.nextAttempt} reason=${event.reason ?? "unknown"}`,
          );
          return;
        }
        ...
        if (event.channel === "pre_tool_use_block") {
          writeStreamingMeta(
            `[pre_tool_use_block] ${event.toolName} blocked by ${event.blockedBy ?? "pre_tool_use_blocked"} (source=${event.sourceToolName ?? "unknown"})`,
          );
          return;
        }

        if (event.channel === "runtime_decision" && runtimeDecisionDebug) {
          if (event.kind === "model_decision") {
            writeStreamingMeta(
              `[runtime_decision] iteration=${event.iteration} type=${event.decisionType} finish=${event.finishReason ?? "unknown"} toolCalls=${event.toolCallCount ?? 0} repeated=${event.repeatedToolBatchDetected ?? false} preBlocks=${event.preToolUseBlockDetected ?? false}`,
            );
            return;
          }
          ...
          if (event.kind === "tool_results") {
            writeStreamingMeta(
              `[runtime_decision] iteration=${event.iteration} tool_results ok=${event.okCount} blocked=${event.blockedCount} error=${event.errorCount} guardrails=${event.newGuardrailCount}`,
            );
            return;
          }
        }
  ...
  logConsoleLine(`Runtime decision debug: ${runtimeDecisionDebug ? "on" : "off"}`);
  ...
  if (runtimeDecisionDebug) {
    logConsoleLine(`Decision journal entries: ${result.decisionJournal?.length ?? 0}`);
  }
