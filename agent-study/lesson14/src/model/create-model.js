...
export function createModel({ traceLogger, modelOverride = null } = {}) {
  ...
  const firstByteTimeoutMs = Number.parseInt(
    process.env.LLM_FIRST_BYTE_TIMEOUT_MS ?? String(requestTimeoutMs),
    10,
  );
  const streamIdleTimeoutMs = Number.parseInt(
    process.env.LLM_STREAM_IDLE_TIMEOUT_MS ?? String(requestTimeoutMs),
    10,
  );
  ...
  const model = modelOverride ?? process.env.OPENAI_MODEL ?? "minimax-m2.5";
