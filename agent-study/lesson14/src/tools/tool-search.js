...
function searchToolSpecs(query, maxResults, specs) {
  ...
  const scored = specs
    .map((spec) => {
      ...
      const normalizedKeywords = normalizeToolSearchQuery(
        Array.isArray(spec.searchKeywords) ? spec.searchKeywords.join(" ") : "",
      );
      const haystack =
        `${name} ${family} ${String(spec.description ?? "").toLowerCase()} ${canonicalName} ${normalizedKeywords}`.trim();
      const normalizedHaystack = `${canonicalName} ${family} ${normalizedDescription} ${normalizedKeywords}`.trim();
