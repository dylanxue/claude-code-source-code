export const lessonNotes = {
  architecture: [
    ...
    {
      clawCode:
        "rust/crates/tools/src/lib.rs (Agent / WorkerCreate / WorkerSendPrompt) + runtime/src/worker_boot.rs",
      nodeVersion:
        "src/tools/agent.js + src/tools/worker-*.js + src/core/subagent-runner.js + src/core/worker-control.js + src/core/subagent-types.js",
      takeaway:
        "第十四课开始把 delegation 做成显式 Agent / Worker 控制面：主 agent 不仅可以按 Explore / Plan / general-purpose / Verification 角色创建受限子 runtime，还可以先走 worker ready-handshake，再安全交付 prompt。",
    },
  ],
};
