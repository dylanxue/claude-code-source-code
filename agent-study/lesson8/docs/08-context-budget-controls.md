...
在继续往前推之后，runtime 里也已经开始记录“模型有没有沿着 continuation hint 走”：

- 每轮 `TOOL BATCH` summary 都会带 `continuationAssessment`
- 当前会把结果分成：
  - `followed`
  - `ignored`
  - `mixed`
  - `no_signal`
  - `answered_without_more_tools`（出现在 final summary）
- 如果模型明明拿到了 hint，却继续发明显同类的 broad request，summary warnings 里会出现：
...
- 默认过滤开启后，`grep_text("lesson7")` 只扫描到 36 个文件、命中 51 处、命中 3 个文件
...
- run summary 里已经被标成：
  - `continuationAssessment.overallStatus = "ignored"`
  - warning: `continuation_ignored_detected`
