...
function createRealLesson12Registry() {
...
test("ToolSearch returns claw-code style matches for external knowledge tools", async () => {
  const { toolSearchTool } = createRealLesson12Registry();
  ...
  assert.equal(result.total_deferred_tools, 11);
...
test("ToolSearch works through the real lesson12 registry and finds file-reading tools from descriptions", async () => {
  const { toolRegistry } = createRealLesson12Registry();
...
test("ToolSearch prefers edit and write tools for file mutation queries in the real lesson12 registry", async () => {
  const { toolRegistry } = createRealLesson12Registry();
...
test("ToolSearch can narrow by family-style shell queries in the real lesson12 registry", async () => {
  const { toolRegistry } = createRealLesson12Registry();
