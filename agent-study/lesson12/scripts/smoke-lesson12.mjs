...
async function startLocalWebFixtureServer() {
  const server = createServer((request, response) => {
    ...
    if (url.pathname === "/page") {
      ...
      response.end(
        [
          "<html><head><title>Lesson 12 Fixture</title></head><body>",
          "<h1>Lesson 12 Fixture</h1>",
    ...
    if (url.pathname === "/search") {
      ...
      response.end(
        [
          "<html><body>",
          `<a class="result__a" href="/l/?uddg=${encodeURIComponent(`http://127.0.0.1:${port}/page`)}">Lesson 12 Fixture</a>`,
...
async function run() {
  ...
  assert(/Lesson 12 Fixture/.test(webFetch.result), "WebFetch should summarize the fetched page", webFetch);
  ...
  const webSearch = await webSearchTool.execute(
    {
      query: "lesson12 fixture",
      blocked_domains: ["blocked.example.com"],
    },
    { workspaceRoot },
  );
  const webSearchHits = webSearch.results?.[1]?.content ?? [];
  assert(webSearch.query === "lesson12 fixture", "WebSearch should echo the query", webSearch);
  ...
  const toolSearch = await toolSearchTool.execute(
    {
  ...
  assert(toolSearch.total_deferred_tools >= 3, "ToolSearch should report searchable tool count", toolSearch);
  ...
  assert(Array.isArray(decisionRun.decisionJournal), "runtime should return a decision journal in lesson12", decisionRun);
