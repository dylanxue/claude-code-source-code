...
function formatTimeoutMessage(timeoutState, defaultTimeoutMs) {
  const timeoutMs = timeoutState?.timeoutMs ?? defaultTimeoutMs;

  if (timeoutState?.kind === "first_byte_timeout") {
    return `OpenAI-compatible chat API timed out after ${timeoutMs}ms while waiting for response.`;
  }

  if (timeoutState?.kind === "stream_idle_timeout") {
    return `OpenAI-compatible chat API timed out after ${timeoutMs}ms while waiting for the next response chunk.`;
  }

  return `OpenAI-compatible chat API timed out after ${timeoutMs}ms while reading the response body.`;
}
...
export class OpenAIResponsesModel {
  ...
  async decide({ systemPrompt, messages, tools, iteration, onAssistantEvent = null }) {
    ...
    const attemptCount = Math.max(1, this.retryLimit + 1);
    let payload = null;

    for (let attempt = 1; attempt <= attemptCount; attempt += 1) {
      const traceSuffix = attempt > 1 ? ` ATTEMPT ${attempt}` : "";
      let emittedAssistantEvents = 0;
      const attemptAssistantEvent = onAssistantEvent
        ? (event) => {
            emittedAssistantEvents += 1;
            emitAssistantEvent(onAssistantEvent, event);
          }
        : null;

      if (this.trace) {
        writeTrace(this.traceLogger, `LLM REQUEST ITERATION ${iteration}${traceSuffix}`, {
      ...
      const armTimeout = (kind, timeoutMs) => {
        if (timeoutId) {
        ...
        timeoutId = setTimeout(() => {
          timeoutState = { kind, timeoutMs };
          abortController.abort();
        }, timeoutMs);
      };

      try {
        armTimeout("first_byte_timeout", positiveIntegerOrNull(this.firstByteTimeoutMs) ?? this.requestTimeoutMs);
        ...
        if (!response.ok) {
          ...
          if (this.trace) {
            writeTrace(this.traceLogger, `LLM RESPONSE ITERATION ${iteration}${traceSuffix}`, {
              ok: false,
          ...
          const error = new Error(`OpenAI-compatible chat API error (${response.status}): ${errorText}`);
        ...
        if (this.stream && response.body && contentType.includes("text/event-stream")) {
          ...
          armTimeout(
            "stream_idle_timeout",
            positiveIntegerOrNull(this.streamIdleTimeoutMs) ?? this.requestTimeoutMs,
          );
          await collectSseEvents(
            ...
            (event) => {
              ...
              if (this.trace) {
                writeTrace(
                  this.traceLogger,
                  `LLM STREAM EVENT ITERATION ${iteration}${traceSuffix}`,
          ...
          payload = buildStreamingPayload(events, {
            onAssistantEvent: attemptAssistantEvent,
          });
        } else {
          armTimeout("body_read_timeout", this.requestTimeoutMs);
        ...
        const normalizedError = isAbort
          ? new Error(formatTimeoutMessage(timeoutState, this.requestTimeoutMs))
          : error;
        ...
        const canRetry =
          ...
          Boolean(normalizedError?.retryable ?? isRetryableTransportError(error)) &&
          emittedAssistantEvents === 0;
        ...
        if (this.trace) {
          writeTrace(this.traceLogger, `LLM RESPONSE ITERATION ${iteration}${traceSuffix}`, {
            ...
            retryable: Boolean(normalizedError?.retryable ?? isRetryableTransportError(error)),
            willRetry: canRetry,
            emittedAssistantEvents,
          });
        }
        ...
        emitAssistantEvent(onAssistantEvent, {
          ...
          reason: normalizedError instanceof Error ? normalizedError.message : String(normalizedError),
        });
