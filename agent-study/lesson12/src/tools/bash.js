...
function buildLinuxSandboxLauncher(command, workspaceRoot, sandboxStatus) {
  ...
  const envOverrides = {
    HOME: sandboxHomePath(workspaceRoot),
    TMPDIR: sandboxTmpPath(workspaceRoot),
...
async function prepareCommandExecution(command, workspaceRoot, sandboxStatus, { createDirs }) {
  ...
  if (sandboxStatus.filesystem_active) {
...
export const bashTool = {
  ...
  async execute(input, context) {
    ...
    try {
      ...
      const timedOut = timeoutMs !== null && error?.killed === true && error?.signal === "SIGTERM";
      if (timedOut) {
        return buildBashResult(input, sandboxStatus, {
          stdout: "",
