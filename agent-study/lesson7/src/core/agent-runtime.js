    ...
    for (let iteration = 1; iteration <= this.maxIterations; iteration += 1) {
      ...
      try {
        decision = await this.model.decide({
          systemPrompt: [this.systemPrompt, context.analysisHint].filter(Boolean).join("\n\n"),
