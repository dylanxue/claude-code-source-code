...
function summarizeMatches(matches) {
  ...
  return [
    `一共找到 ${matches.length} 条匹配。`,
    ...matches.slice(0, 5).map((match) => `- ${match.path}:${match.line}: ${match.preview}`),
  ].join("\n");
}
...
function summarizeToolResult(lastToolResult) {
  ...
  if (payload?.path && payload?.preview) {
    const summary = payload.path.endsWith(".md")
      ? summarizeMarkdownPreview(payload.preview)
      : summarizeTextPreview(payload.preview);
  ...
  if (payload?.matches) {
    return [
      ...
      "",
      summarizeMatches(payload.matches),
      "",
...
export class MockModel {
  async decide({ messages, iteration }) {
    ...
    if (iteration >= 2) {
      if (
        lastToolResult.content?.toolName === "grep_text" &&
        lastToolResult.content?.ok &&
        lastToolResult.content?.content?.matches?.length &&
        (userText.includes("阅读") || userText.includes("总结") || userText.toLowerCase().includes("read"))
      ) {
        const firstMatch = lastToolResult.content.content.matches[0];
        return {
          type: "tool",
          toolName: "read_file",
          input: { path: firstMatch.path },
        };
      }
