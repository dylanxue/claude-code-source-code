...
export function contextWindowUsage({ contextWindowTokens, currentUsage }) {
  ...
  const usedTokens = Number(currentUsage?.inputTokens ?? 0)
    + Number(currentUsage?.cacheCreationInputTokens ?? 0)
    + Number(currentUsage?.cacheReadInputTokens ?? 0);
  const usedPercentage = Math.min(100, Math.round((usedTokens / contextWindowTokens) * 100));
