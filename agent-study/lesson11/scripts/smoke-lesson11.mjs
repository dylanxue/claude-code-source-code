...
async function startLocalWebFixtureServer() {
  const server = createServer((request, response) => {
    ...
    if (url.pathname === "/page") {
      ...
      response.end(
        [
          "<html><head><title>Lesson 11 Fixture</title></head><body>",
          "<h1>Lesson 11 Fixture</h1>",
    ...
    if (url.pathname === "/search") {
      ...
      response.end(
        [
          "<html><body>",
          `<a class="result__a" href="/l/?uddg=${encodeURIComponent(`http://127.0.0.1:${port}/page`)}">Lesson 11 Fixture</a>`,
...
async function run() {
  ...
  assert(/Lesson 11 Fixture/.test(webFetch.result), "WebFetch should summarize the fetched page", webFetch);
  ...
  assert(webSearch.query === "lesson11 fixture", "WebSearch should echo the query", webSearch);
  ...
  assert(Array.isArray(decisionRun.decisionJournal), "runtime should return a decision journal in lesson11", decisionRun);
